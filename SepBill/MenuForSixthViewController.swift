//
//  MenuForSixthViewController.swift
//  SepBill
//
//  Created by Кирилл Сысоев on 02.11.2024.
//

import UIKit

protocol MenuForSixthClientViewControllerDelegate: AnyObject {
    func updateSixthClientBill(for tableNumber: Int, with client6Bill: Double)
}

class MenuForSixthViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {

    weak var delegate: MenuForSixthClientViewControllerDelegate?
    ...
    var client6Bill: Double = 0 {
        didSet {
            billLabel.text = "Счет: \(client6Bill) р."
        }
    }
    ...
    func loadSelectedProducts() {
        if let savedProductData = UserDefaults.standard.dictionary(forKey: "productQuantitiesForTable_\(tables[selectedTableIndex])_client6") as? [String: Int] {
    ...
    func saveSelectedProducts() {
        ...
        UserDefaults.standard.set(selectedProductNames, forKey: "selectedProductsForSixthClient_\(tables[selectedTableIndex])")
        
        // Сохраняем количества
        let productQuantitiesToSave = productQuantities.mapKeys { $0.productName }
        UserDefaults.standard.set(productQuantitiesToSave, forKey: "productQuantitiesForTable_\(tables[selectedTableIndex])_client6")
    }
    ...
    @objc func selectProduct(_ sender: UISwitch) {
        ...
        if sender.isOn {
            ...
            selectedProducts.append(product)
            client6Bill += product.productPrice // Обновляем сумму счета второго клиента
            ...
            if let index = selectedProducts.firstIndex(of: product) {
                ...
                client6Bill -= product.productPrice // Уменьшаем сумму счета второго клиента
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "backToMain" {
            ...
            delegate?.updateSixthClientBill(for: tableNumber, with: client6Bill) // Передаем счет
        }
    }
}
