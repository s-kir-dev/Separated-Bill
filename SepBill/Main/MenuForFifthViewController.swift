//
//  MenuForFifthViewController.swift
//  SepBill
//
...
protocol MenuForFifthClientViewControllerDelegate: AnyObject {
    func updateFifthClientBill(for tableNumber: Int, with client5Bill: Double)
}

class MenuForFifthViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {

    weak var delegate: MenuForFifthClientViewControllerDelegate?
    ...
    var client5Bill: Double = 0 {
        didSet {
            billLabel.text = "Счет: \(client5Bill) р."
        }
    }
    ...
    func loadSelectedProducts() {
        if let savedProductData = UserDefaults.standard.dictionary(forKey: "productQuantitiesForTable_\(tables[selectedTableIndex])_client5") as? [String: Int] {
    ...
    func saveSelectedProducts() {
        ...
        UserDefaults.standard.set(selectedProductNames, forKey: "selectedProductsForFifthClient_\(tables[selectedTableIndex])")
        
        // Сохраняем количества
        let productQuantitiesToSave = productQuantities.mapKeys { $0.productName }
        UserDefaults.standard.set(productQuantitiesToSave, forKey: "productQuantitiesForTable_\(tables[selectedTableIndex])_client5")
    }
    ...
    @objc func selectProduct(_ sender: UISwitch) {
        ...
        if sender.isOn {
            ...
            selectedProducts.append(product)
            client5Bill += product.productPrice // Обновляем сумму счета второго клиента
            ...
            if let index = selectedProducts.firstIndex(of: product) {
                ...
                client5Bill -= product.productPrice // Уменьшаем сумму счета второго клиента
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "backToMain" {
            ...
            delegate?.updateFifthClientBill(for: tableNumber, with: client5Bill) // Передаем счет
        }
    }
}
